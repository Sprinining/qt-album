...
ProTreeItem::ProTreeItem(QTreeWidget *treeview, const QString &name,
    ...
    : QTreeWidgetItem(treeview, static_cast<int>(type)), path(path), name(name),
    item_root(this), item_prev(nullptr), item_next(nullptr) {}
...
ProTreeItem::ProTreeItem(QTreeWidgetItem *parent, const QString &name,
    ...
    : QTreeWidgetItem(parent, static_cast<int>(type)), path(path), name(name),
    item_root(root), item_prev(nullptr), item_next(nullptr) {}

const QString &ProTreeItem::getPath() const { return path; }

const QTreeWidgetItem *ProTreeItem::getRoot() const { return item_root; }
...
void ProTreeItem::setItemPrev(QTreeWidgetItem *prev) { item_prev = prev; }

void ProTreeItem::setItemNext(QTreeWidgetItem *next) { item_next = next; }
